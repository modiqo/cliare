...
The implemented `measure` command fingerprints a target binary, runs bounded safe probes inside an isolated HOME/PWD/XDG/TMP sandbox with a sanitized environment, records `evidence.jsonl`, emits a generic `shape.json`, and writes experimental `scorecard.json` and `report.md` artifacts over currently measured dimensions. The shape artifact now includes aliases, usage-derived positionals, and flag grammar such as boolean, required-value, optional-value, repeatable, and required flags where help output exposes them. It also writes `measure-cache.json`; later runs reuse artifacts when the target fingerprint, traversal profile, sandbox profile, resolved probe budget, expected-value threshold, CLIARE version, and measurement engine match. Use `--refresh` to force a new probe run. The implemented `guard` command measures a target and fails on total-score regression against a baseline scorecard. Traversal profiles provide useful presets: `quick` is depth 3 / 64 probes, `standard` is depth 5 / 256 probes, and `deep` is depth 8 / 1000 probes. `--max-depth`, `--max-probes`, and `--min-expected-value` override the selected profile for larger, tighter, or more aggressive CI runs. Scorecards also report coverage pressure and runtime isolation metadata, including profile, observed depth, frontier remaining, expected-value convergence skips, candidates skipped by depth, stop reason, probes skipped by budget, sandbox root, and env policy. Other commands remain planned.
